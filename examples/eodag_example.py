...
if __name__ == "__main__":
    ...
    print(list_collections(guess="S2"))

    for site in site_def.keys():
        query = {
            "collections": [
                "S2_MSI_L1C"
            ],  # ["S2_MSI_L1C", "S3_EFR", "S3_SLSTR_L1RBT", "LANDSAT_C2L1"]
            # "platform": "S2A",
            "geom": site_def[site],
            "start_time": start,
            "stop_time": end,
        ...
        print(products)
        # products.filter_id_contains("T33KWP")  # filter only T33KWP tile
        # products.plot_geometries()
        # plt.show()

        # with open("productItemSet_2022_S2A.pkl", "wb") as f:
